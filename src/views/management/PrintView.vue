...
// import { getuserdetail } from '@/api/setting'
// import { getUserDetailById } from '@/api/user'
import { userinfo, gtejob } from '@/api/management'
export default {
  data() {
    return {
      printObj: {
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getUserInfo(this.userId)
      // console.log(this.formData)// 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await userinfo(this.userId)
      console.log(userInfo)
      const jobInfo = await gtejob(this.userId) // 获取个人基本信息
      this.formData = { ...jobInfo.data, ...userInfo.data }
